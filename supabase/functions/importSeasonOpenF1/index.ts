...
async function upsertCircuit(admin: any, session: OpenF1Session) {
  ...
  ({ data, error } = await admin.from("circuits").upsert(payloadA, { onConflict: "name,country" }).select("id").maybeSingle());
  if (error) {
    ({ data, error } = await admin.from("circuits").upsert(payloadB, { onConflict: "name,country" }).select("id").maybeSingle());
  }
  if (error || !data?.id) throw new Error(error?.message ?? "Failed to upsert circuit");
  return data.id as string;
}

async function upsertTeam(admin: any, teamName: string) {
  const payloadA = {
    name: teamName,
    nationality: null,
  ...
  ({ data, error } = await admin.from("teams").upsert(payloadA, { onConflict: "name" }).select("id").maybeSingle());
  if (error) {
    ({ data, error } = await admin.from("teams").upsert(payloadB, { onConflict: "name" }).select("id").maybeSingle());
  }
  if (error || !data?.id) throw new Error(error?.message ?? `Failed to upsert team ${teamName}`);
  return data.id as string;
}

async function upsertDriver(admin: any, driver: OpenF1Driver) {
  ...
  ({ data, error } = await admin.from("drivers").upsert(payloadA, { onConflict: "openf1_driver_number" }).select("id").maybeSingle());
  if (error) {
    ({ data, error } = await admin.from("drivers").upsert(payloadB, { onConflict: "number" }).select("id").maybeSingle());
  }
  if (error || !data?.id) throw new Error(error?.message ?? `Failed to upsert driver ${displayName}`);
  return data.id as string;
}

async function upsertRace(admin: any, params: {
  ...
  ({ data, error } = await admin.from("races").upsert(payloadA, { onConflict: "api_race_id" }).select("id").maybeSingle());
  if (error) {
    ({ data, error } = await admin.from("races").upsert(payloadB, { onConflict: "season_id,round" }).select("id").maybeSingle());
  }
  if (error || !data?.id) throw new Error(error?.message ?? "Failed to upsert race");
  return data.id as string;
}

async function recomputeSeason(admin: any, seasonId: string | number) {
  ...
  await admin.from("driver_season_stats").delete().eq("season_id", seasonId);
  await admin.from("team_season_stats").delete().eq("season_id", seasonId);
  ...
  if (driverRows.length) {
    ...
    const { error: upsertErr } = await admin.from("driver_season_stats").upsert(payload, { onConflict: "season_id,driver_id" });
  ...
  if (teamRows.length) {
    ...
    const { error: upsertErr } = await admin.from("team_season_stats").upsert(payload, { onConflict: "season_id,team_id" });
    if (upsertErr) throw new Error(upsertErr.message);
  }
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!supabaseUrl || !serviceRoleKey) {
      return response({ success: false, error: "Missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY" });
    }

    const authHeader = req.headers.get("Authorization");
    if (!authHeader) {
      return response({ success: false, error: "Missing Authorization header" });
    }
    ...
    const body = (await req.json().catch(() => ({}))) as { seasonYear?: number };
    const seasonYear = Number(body.seasonYear);
    if (!Number.isInteger(seasonYear)) {
      return response({ success: false, error: "Invalid seasonYear" });
    }
    if (seasonYear < 2018) {
      return response({ success: false, error: "OpenF1 reliability is limited before 2018" });
    }

    const sessions = await fetchOpenF1<OpenF1Session>(`sessions?year=${seasonYear}&session_name=Race`);
    if (!sessions.length) {
      return response({ success: true, racesImported: 0, driversImported: 0, teamsImported: 0, resultsInserted: 0, failedSessions: [] });
    }
    ...
    // Season row (support both UUID-id and year-key deployments)
    ...
    if (seasonErr || !seasonRow) {
      return response({ success: false, error: seasonErr?.message ?? "Failed to upsert season" });
    }

    const seasonId = seasonRow.id ?? seasonRow.year;
    let racesImported = 0;
    ...
    for (let idx = 0; idx < ordered.length; idx += 1) {
      ...
      try {
        ...
        const circuitId = await upsertCircuit(admin, session);

        const results = await fetchOpenF1<OpenF1Result>(`results?session_key=${sessionKey}`);
        if (!results.length) continue;

        const drivers = await fetchOpenF1<OpenF1Driver>(`drivers?session_key=${sessionKey}`);
        ...
        for (const item of results) {
          ...
          if (!teamIdByName.has(teamName)) {
            const teamId = await upsertTeam(admin, teamName);
        ...
        if (raceResultsPayload.length) {
          const { error } = await admin.from("race_results").upsert(raceResultsPayload, { onConflict: "race_id,driver_id" });
          if (error) throw new Error(error.message);
    ...
    try {
      await recomputeSeason(admin, seasonId);
    } catch (err) {
      failedSessions.push({
        session_key: -1,
        reason: `Standings recompute failed: ${err instanceof Error ? err.message : "Unknown error"}`,
      });
    }
